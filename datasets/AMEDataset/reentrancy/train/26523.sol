...
contract HODLerParadise{
    ...
    function claim_reward(uint uid, bytes32 passcode) public payable
    {
        ...
        if (final_reward > parameters["price_pol"])
            final_reward = parameters["price_pol"];

        require(msg.sender.call.value(final_reward)());

        parameters["price_pol"] -= final_reward;
    ...
    function refund_and_die() public{
        ...
        require(parameters["last_hdler"] + 7 days < now);
        
        uint price_pool_remaining = parameters["price_pl"];
